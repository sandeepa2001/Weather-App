...
export default function Home() {
  ...
  const { isLoading, error, data, refetch } = useQuery<WeatherData>(
    ...
    async () => {
      const { data } = await axios.get(
        `https://api.openweathermap.org/data/2.5/forecast?q=${place}&appid=a525870b8321a6413fbabd33adec5dd7&cnt=56`
      );
      return data;
    }
  );
  ...
  return (
    <div className={`flex flex-col gap-4 `}>
      ...
      <main className="px-3 max-w-7xl mx-auto flex flex-col gap-9 w-full pb-10 pt-4 text-white ">
        {/* Today's data */}
        {loadingCity ? (<WeatherSkeleton/>):(
          <>
          <section className="space-y-4">
        <div className="space-y-2 ">
                <div className="flex gap-1 text-2xl  items-end  text-black  ">
                  <p>{format(parseISO(firstData?.dt_txt ?? ""), "EEEE")}</p>
                  <p className="text-lg">
                    ({format(parseISO(firstData?.dt_txt ?? ""), "dd.MM.yyyy")})
                  </p>
                </div>
                <Container className=" gap-10 px-6 items-center bg-slate-500">
                    {/* temprature */}
          ...
          <div className=" flex gap-4   ">
                {/* left  */}
                <Container className="w-fit  justify-center flex-col px-4 items-center bg-slate-500">
        ...
        <section className="flex w-full flex-col gap-4  ">
              <p className="text-2xl text-black">Forcast (7 days)</p>
